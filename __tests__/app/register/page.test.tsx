...
describe("RegisterPage", () => {
  ...
  it("renders the registration form", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Join Noteworthy/i }),
    ).toBeInTheDocument();
  ...
  it("shows API error with details on failed API response with details", async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ...
      json: async () => ({
        error: "Invalid input",
        details: { email: ["Bad email format"], password: ["Too short"] },
      }),
    });
    ...
    expect(
      await screen.findByText(
        /Invalid input Email: Bad email format. Password: Too short./i,
      ),
    ).toBeInTheDocument();
  });
