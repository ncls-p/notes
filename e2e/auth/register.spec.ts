...
test.describe("Registration Flow", () => {
  ...
  test("should allow a new user to register successfully and redirect to login", async ({
    page,
  }) => {
    await page.goto("/register");

    await expect(
      page.getByRole("heading", { name: "Join Noteworthy" }),
    ).toBeVisible();
  ...
  test("should show an error message if trying to register with an existing email", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(
        /Email already registered|Invalid input Email: Email already registered/i,
      ),
